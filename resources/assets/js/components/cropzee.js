    ...
    cropzee: function (options = {
        ...
        function cropzeeTriggerModal(id, src) {
            ...
            var lightmodalHTML =
                ...
                + '</div> -->'
                + '<!-- light modal body -->'
                + '<div class="light-modal-body" style="max-height: 600px;background: #F8F6F2">'
                + '<img id="cropzee-modal-image" src="' + src + '">'
                + '</div>'
                + '<!-- light modal footer -->'
                + '<div class="light-modal-footer" style="justify-content: space-between;">'
                + '<div onclick="closeModal()" class="light-modal-close-btn" style="cursor: pointer;" aria-label="close">Đóng</div>'
                + '<div onclick="cropzeeRotateImage(`' + id + '`);" class="light-modal-close-btn" style="cursor: pointer;">Rotate 90deg</div>'
                + '<div onclick="cropzeeCreateImage(`' + id + '`);" class="light-modal-close-btn" style="cursor: pointer;">Tiếp theo</div>'
                + '</div>'
                + '</div>'
