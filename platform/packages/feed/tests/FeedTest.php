<?php namespace Workable\Feed\Test;

use Orchestra\Testbench\TestCase as TestCase;
use Workable\Feed\FeedRss;
use Workable\Feed\FeedServiceProvider;
use Carbon\Carbon;
...
class FeedTest extends TestCase
{
    ...
    public function setUp() : void
    {
        ...
        $this->feed = $this->app->make(FeedRss::class);
    }

    public function testMisc()
    {
        // test object initialization
        $this->assertInstanceOf(FeedRss::class, $this->feed);

        // test custom methods
        $this->assertEquals([FeedServiceProvider::class], $this->getPackageProviders($this->feed));
        $this->assertEquals(['Feed'=>FeedRss::class], $this->getPackageAliases($this->feed));

        // test FeedServiceProvider (fixes coverage of the class!)
        $this->sp = new FeedServiceProvider($this->feed);
        $this->assertEquals(['feed', FeedRss::class], $this->sp->provides());
    }
    ...
    {
        // default formats
        $this->assertEquals('<link rel="alternate" type="application/atom+xml" href="http://domain.tld/feed">', FeedRss::link('http://domain.tld/feed', 'atom'));
        $this->assertEquals('<link rel="alternate" type="application/rss+xml" href="http://domain.tld/feed">', FeedRss::link('http://domain.tld/feed', 'rss'));

        // with custom type
        $this->assertEquals('<link rel="alternate" type="text/xml" href="http://domain.tld/feed">', FeedRss::link('http://domain.tld/feed', 'text/xml'));

        // with title
        $this->assertEquals('<link rel="alternate" type="application/rss+xml" href="http://domain.tld/feed" title="Feed: RSS">', FeedRss::link('http://domain.tld/feed', 'rss', 'Feed: RSS'));

        // with title and lang
        $this->assertEquals('<link rel="alternate" hreflang="en" type="application/atom+xml" href="http://domain.tld/feed" title="Feed: Atom">', FeedRss::link('http://domain.tld/feed', 'atom', 'Feed: Atom', 'en'));
    }
